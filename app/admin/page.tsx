...
const DEPT_GROUPS: { label: string; match: (dept: string) => boolean }[] = [
  ...
  {
    label: "조이랜드",
    match: (d) => d.includes("조이랜드") || d.includes("초등"),
  },
  {
    label: "조이코너",
    match: (d) => d.includes("조이코너") || d.includes("유치"),
  },
  {
    label: "조이베이비",
    match: (d) =>
      d.includes("조이베이비") || d.includes("영아") || d.includes("베이비"),
  },
];

function calcDeptStats(rows: Row[]) {
  return DEPT_GROUPS.map(({ label, match }) => {
    const members = rows.filter((r) => match(r.소속) && r.구분 !== "할인");
    const count      = members.length;
    const lodging    = members.filter((r) => r.숙박 === "숙박").length;
    const nonLodging = count - lodging;
    const paidPct    = count > 0 ? Math.round(members.filter((r) => r.입금확인 === "확인").length / count * 100) : 0;
    return { label, count, lodging, nonLodging, paidPct };
  });
}
...
function LodgeBadge({ value }: { value: string }) {
  return (
    <span
      className={`inline-block rounded-sm px-2 py-0.5 text-xs font-semibold ${value === "숙박" ? "bg-blue-100 text-blue-700" : "bg-slate-100 text-slate-500"}`}
    >
      {value}
    </span>
  );
...
function SortIcon({ dir }: { dir: SortDir }) {
  if (dir === "asc")
    return <span className="ml-0.5 text-blue-500 text-[9px]">▲</span>;
  if (dir === "desc")
    return <span className="ml-0.5 text-blue-500 text-[9px]">▼</span>;
...
function cellCode(s: string): string {
  return s?.split(" ")[0] ?? "";
}
...
function IndividualTable({
  ...
  const sorted = useMemo(() => {
    ...
    return [...groups].sort((a, b) => {
      switch (sortKey) {
        case "status":
          return (
            ((a.confirmed ? 2 : a.paid ? 1 : 0) -
              (b.confirmed ? 2 : b.paid ? 1 : 0)) *
            m
          );
        case "name":
          return a.rep.이름.localeCompare(b.rep.이름, "ko") * m;
        case "dept":
          return a.rep.소속.localeCompare(b.rep.소속, "ko") * m;
        case "regType":
          return a.rep.등록유형.localeCompare(b.rep.등록유형, "ko") * m;
        case "lodging":
          return a.rep.숙박.localeCompare(b.rep.숙박, "ko") * m;
        case "cell":
          return (
            cellCode(a.rep.셀번호).localeCompare(cellCode(b.rep.셀번호)) * m
          );
        case "amount":
        ...
        case "confirmed":
          return ((a.confirmed ? 1 : 0) - (b.confirmed ? 1 : 0)) * m;
        default:
          return 0;
      }
    });
  ...
  const plainTh = (
    ...
    <th
      className={`border-b border-slate-200 px-3 py-2.5 whitespace-nowrap text-slate-500 text-${align}`}
    >
      {label}
    </th>
  );
          ...
          <tbody>
            {sorted.map((g) => {
              ...
              return (
                <tr
                  key={g.groupId}
                  className={`${g.confirmed ? "bg-emerald-50/40" : "bg-white hover:bg-slate-50"} border-b border-slate-100`}
                >
                  ...
                  <td className="px-3 py-2.5 text-center text-slate-500">
                    {cellDisplay(rep.셀번호, rep.소속)}
                  </td>
                  <td className="px-3 py-2.5 text-center whitespace-nowrap text-slate-600">
                    {rep.등록유형}
                  </td>
                  <td className="px-3 py-2.5 text-center">
                    <LodgeBadge value={rep.숙박} />
                  </td>
...
function GroupCard({
  ...
  return (
    <div
      className={`rounded-sm border ${g.confirmed ? "border-emerald-200" : "border-slate-200"}`}
    >
      {/* 그룹 헤더 */}
      <div
        className={`flex items-center justify-between gap-3 px-4 py-3 ${g.confirmed ? "bg-emerald-50/60" : "bg-slate-50"}`}
      >
        <button
          type="button"
          onClick={() => setOpen((v) => !v)}
          className="flex items-center gap-3 flex-wrap text-left min-w-0"
        >
          <svg
          ...
          {g.wantsFamilyRoom && (
            <span className="inline-flex items-center rounded-sm border border-blue-200 bg-blue-50 px-2 py-0.5 text-xs font-semibold text-blue-700">
              🛏 가족실
            </span>
          )}
        ...
        <div className="flex items-center gap-2 shrink-0">
          <span className="font-bold text-slate-800 mr-1 whitespace-nowrap">
            {krw(g.total)}
          </span>
          {g.paid ? (
          ...
          <DeleteButton
            groupId={g.groupId}
            label={`${g.rep.이름} 외 ${memberCount - 1}명`}
            password={password}
            onDelete={onDelete}
          />
        </div>
              ...
              row.구분 === "할인" ? (
                ...
                <tr key={i} className="border-t border-slate-100">
                  <td className="px-4 py-2">
                    <span
                      className={`inline-block rounded-sm px-2 py-0.5 text-xs font-semibold ${row.구분 === "대표자" ? "bg-blue-100 text-blue-700" : "bg-slate-100 text-slate-500"}`}
                    >
                      {row.구분}
                    </span>
                  </td>
  ...
  const sorted = useMemo(() => {
    ...
    return [...groups].sort((a, b) => {
      switch (sortKey) {
        ...
        case "count":
          return (
            (a.rows.filter((r) => r.구분 !== "할인").length -
              b.rows.filter((r) => r.구분 !== "할인").length) *
            m
          );
        case "amount":
          return (a.total - b.total) * m;
      ...
      <table className="w-full text-sm border-collapse">
        <thead>
          <tr className="bg-slate-50 text-xs text-slate-500">
            <th className="border-b border-slate-200 px-3 py-2.5 text-left whitespace-nowrap">
              대표자
            </th>
...
function DeptMemberTable({
  ...
  const raw = groups.flatMap((g) =>
    g.rows.filter((r) => match(r.소속) && (!lodgingFilter || r.숙박 === lodgingFilter)).map((r) => ({ r, g })),
  );
  ...
  const members = useMemo(() => {
    ...
    return [...raw].sort((a, b) => {
      switch (sortKey) {
        case "name":
          return a.r.이름.localeCompare(b.r.이름, "ko") * m;
        case "cell":
          return cellCode(a.r.셀번호).localeCompare(cellCode(b.r.셀번호)) * m;
        case "regType":
          return a.r.등록유형.localeCompare(b.r.등록유형, "ko") * m;
        case "lodging":
          return a.r.숙박.localeCompare(b.r.숙박, "ko") * m;
        case "status":
          return (
            ((a.g.confirmed ? 2 : a.g.paid ? 1 : 0) -
              (b.g.confirmed ? 2 : b.g.paid ? 1 : 0)) *
            m
          );
        default:
          return 0;
        ...
        <tbody>
          {members.map(({ r, g }, i) => (
            <tr key={i} className="border-b border-slate-100 hover:bg-slate-50">
              ...
              <td className="px-3 py-2.5 text-center">
                <span
                  className={`inline-block rounded-sm px-2 py-0.5 text-xs font-semibold ${g.type === "개인" ? "bg-slate-100 text-slate-500" : r.구분 === "대표자" ? "bg-blue-100 text-blue-700" : "bg-slate-100 text-slate-500"}`}
                >
                  {g.type === "개인" ? "개인" : r.구분}
                </span>
              </td>
...
function calcEditPrices(editRows: EditRow[]): {
  ...
  const prices = editRows.map((m) => {
    ...
    if (m.등록유형 === "마프할인") return Math.floor((MINISTRY_PRICE[m.소속] ?? 0) / 2);
    if (m.등록유형 === "사역자") return MINISTRY_PRICE[m.소속] ?? 0;
    const regType = REG_TYPE_TO_CODE[m.등록유형] as
      | RegistrationType
      | undefined;
    if (!regType) return 0;
  ...
  for (const tier of MULTI_CHILD_DISCOUNT.tiers) {
    if (childCount >= tier.minChildren) {
...
function EditModal({
  ...
  const { prices, discount, total } = useMemo(
    () => calcEditPrices(editRows),
    [editRows],
  );
            ...
            const rowCls = isMinistry
              ...
              : isMarf
                ...
                : isMissionary
                  ...
                  : "border-slate-200";
            const selectCls = isMinistry
              ? "border-purple-300 bg-purple-50 text-purple-700 focus:border-purple-500"
              : isMarf
                ? "border-teal-300 bg-teal-50 text-teal-700 focus:border-teal-500"
                : isMissionary
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    {(isMinistry || isMarf) && (
                      <button
                        ...
                        className={`shrink-0 rounded px-1.5 py-0.5 text-[10px] font-bold transition-colors ${isMarf ? "bg-teal-500 text-white" : "border border-teal-300 text-teal-600 hover:bg-teal-50"}`}
                      >
                        마프
                      </button>
                    )}
...
function DeleteButton({
  ...
  async function handleClick() {
    if (
      !confirm(
        `'${label}' 신청을 삭제할까요?\n\n구글시트(엑셀)에서도 함께 삭제되며 되돌릴 수 없습니다.`,
      )
    )
      return;
    setLoading(true);
...
function exportExcel(rows: Row[]) {
  ...
  XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([EXCEL_HEADERS, ...rows.map(rowToArr)]), "전체");
...
function Dashboard({
  ...
  const [lodgingFilter, setLodgingFilter] = useState<"숙박" | "비숙박" | null>(null);
  ...
  const load = useCallback(
    async (silent = false) => {
      ...
      try {
        ...
        setRows(data);
        setConfirmed(
          new Set(data.filter((r) => r.확정 === "확정").map((r) => r.groupId)),
        );
        setPaid(
          new Set(
            data.filter((r) => r.입금확인 === "확인").map((r) => r.groupId),
          ),
        );
      } catch {
  ...
  const groups = useMemo(
    () => buildGroups(rows, confirmed, paid),
    [rows, confirmed, paid],
  );
  ...
  function handleConfirm(id: string) {
    setConfirmed((p) => new Set([...p, id]));
  }
  function handlePaid(id: string) {
    setPaid((p) => new Set([...p, id]));
  }
  function handleDelete(id: string) {
    setRows((prev) => prev.filter((r) => r.groupId !== id));
  }
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-auto bg-slate-50">
      ...
      <div className="max-w-screen-2xl mx-auto px-6 py-6 flex flex-col gap-6">
        {error && (
          <div className="rounded-sm bg-red-50 border border-red-200 px-4 py-3 text-sm text-red-600">
            {error}
          </div>
        )}
        {loading && (
          ...
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
              ...
              <KpiCard
                ...
                value={paidCnt}
                unit="건"
                sub={`미확인 ${totalGroups - paidCnt}건`}
              />
              <KpiCard
                label="예상 회비"
                value={(expectedTotal / 10000).toLocaleString()}
                unit="만원"
                sub="할인 반영 합계"
              />
            </div>
                  ...
                  {deptStats.map(({ label, count }) => {
                    ...
                    return (
                      <button
                        key={label}
                        type="button"
                        onClick={() => { setDeptFilter(active ? null : label); setLodgingFilter(null); }}
                      ...
                      >
                        <p
                          className={`text-xs font-medium whitespace-nowrap ${active ? "text-blue-600" : "text-slate-500"}`}
                        >
                          {label}
                        </p>
                        <p
                          className={`text-2xl font-bold mt-1 ${active ? "text-blue-700" : "text-slate-900"}`}
                        >
                          {count}
                        </p>
                      </button>
            ...
            {activeDept && (
              <div className="rounded-sm bg-white border border-blue-200 overflow-hidden">
                <div className="flex items-center justify-between px-4 py-3 bg-blue-50 border-b border-blue-100">
                  <h3 className="text-sm font-bold text-blue-800 flex flex-wrap items-center gap-x-3 gap-y-1">
                    <span>{activeDept.label} 부서 명단</span>
                    {(() => {
                      const members = rows.filter((r) => activeDept.match(r.소속) && r.구분 !== "할인");
                      ...
                      return (
                        <>
                          ...
                          <div className="inline-flex rounded-lg border border-slate-300 overflow-hidden">
                            <button type="button" onClick={() => setLodgingFilter(lodgingFilter === "숙박" ? null : "숙박")}
                              className={`px-2.5 py-0.5 text-sm font-semibold transition-colors ${lodgingFilter === "숙박" ? "bg-blue-600 text-white" : "bg-white text-slate-700 hover:bg-slate-50"}`}>
                              숙박 {lodging}명
                            </button>
                            <div className="w-px bg-slate-300" />
                            <button type="button" onClick={() => setLodgingFilter(lodgingFilter === "비숙박" ? null : "비숙박")}
                              className={`px-2.5 py-0.5 text-sm font-semibold transition-colors ${lodgingFilter === "비숙박" ? "bg-blue-600 text-white" : "bg-white text-slate-700 hover:bg-slate-50"}`}>
                              비숙박 {total - lodging}명
                            </button>
                          </div>
                  ...
                  <button type="button" onClick={() => { setDeptFilter(null); setLodgingFilter(null); }}
                    className="text-xs font-semibold text-slate-500 hover:text-slate-700">✕ 닫기</button>
                </div>
                <div className="p-4">
                  <DeptMemberTable groups={groups} match={activeDept.match} lodgingFilter={lodgingFilter} />
                </div>
              </div>
            )}
                    ...
                    <>
                      {filtered.some((g) => g.type === "개인") && (
                        <>
                          ...
                          <IndividualTable
                            groups={filtered.filter((g) => g.type === "개인")}
                            password={password}
                            onConfirm={handleConfirm}
                            onPaid={handlePaid}
                            onDelete={handleDelete}
                      ...
                      {filtered.some((g) => g.type === "단체") && (
                        ...
                        >
                          ...
                          <GroupTable
                            groups={filtered.filter((g) => g.type === "단체")}
                            password={password}
                            onConfirm={handleConfirm}
                            onPaid={handlePaid}
                            onDelete={handleDelete}
